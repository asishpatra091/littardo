				...
				<td>
					<?php if(Auth::user()->user_type == 'seller'): ?>
						<?php if(Auth::user()->shop->logo != null): ?>
							<img src="<?php echo e(asset(Auth::user()->shop->logo)); ?>" height="40" style="display:inline-block;">
						<?php else: ?>
        ...
        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
	        <tbody>
		        <tr>
		            ...
		            <td><?php echo e(single_price($order->orderDetails->where('seller_id', Auth::user()->id)->sum('price'))); ?></td>
		        </tr>
		        <tr>
		            <th class="gry-color text-left">Shipping Cost</td>
		            <td><?php echo e(single_price($order->orderDetails->where('seller_id', Auth::user()->id)->sum('shipping_cost'))); ?></td>
		        </tr>
		        <tr class="border-bottom">
		            <th class="gry-color text-left">Total Tax</td>
		            <td><?php echo e(single_price($order->orderDetails->where('seller_id', Auth::user()->id)->sum('tax'))); ?></td>
		        </tr>
		        <tr>
		            <th class="text-left strong">Grand Total</td>
		            <td><?php echo e(single_price($order->orderDetails->where('seller_id', Auth::user()->id)->sum('price') + $order->orderDetails->where('seller_id', Auth::user()->id)->sum('tax') + $order->orderDetails->where('seller_id', Auth::user()->id)->sum('tax'))); ?></td>
		        </tr>
	        </tbody>
	    </table>
    </div>
...
<?php /**PATH /home/littardoemporium/public_html/shop/resources/views/invoices/seller_invoice.blade.php ENDPATH**/ ?>